...
public class TransparentOverlay : Form
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        lock (_lockObj)
        {
            ...
            {            Console.WriteLine(effect.Type);
                ...
                else if (effect.Type == typeMouse)
                {
                    // アイコンの描画
                    //e.Graphics.DrawImage(effect.Icon, effect.Location);
                }
                else if (effect.Type == typeModiKey)
                    {
                        using (var font = new Font("Yu Gothic UI", 30, FontStyle.Bold))
                    using (var brush = new SolidBrush(Color.Cyan))
                    {
                        var location = new Point(effect.Location.X + 20, effect.Location.Y);
                        e.Graphics.DrawString(effect.Text, font, brush, location);
                    }}
            }
        }
    }
    ...
    private class ClickEffect
    {
        ...
        public ClickEffect(string text, Point location)
        {
            Text = text;
            Location = location;
            Type = 2;
    ...
    private class KeyStateDisplay
    {
        public string Text { get; set; }
        public Point Location {get; set;}
        public Color Color { get; set;}

        public KeyStateDisplay(string text, Point location, Color color)
        {
            Text = text;
            Location = location;
            Color = color;
    ...
    private IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        ...
        if (nCode >= 0)
        {
            ...
            var modKey = "";
            if ((Control.ModifierKeys & Keys.Control) > 0) modKey += "CONTROL";
            if ((Control.ModifierKeys & Keys.Shift) > 0) modKey += "\nSHIFT";
            if (modKey != "") 
            {
                var keyEffect = new ClickEffect(modKey, clickPos);
                lock (_lockObj)
                {
                ...
                Task.Delay(800).ContinueWith(t => 
                {
                    lock (_lockObj)
                    {
                        _activeEffects.Remove(keyEffect);
                    }
                    this.Invalidate();
                }, TaskScheduler.FromCurrentSynchronizationContext());
            }
...
/*
    protected override void WndProc(ref Message msg)
    {
        ...
        if (msg.Msg == KEYDOWN || msg.Msg == KEYUP || msg.Msg == WM_SYSKEYDOWN || msg.Msg == WM_SYSKEYUP)
        {
            CheckModifierKeys();
        }
    ...
    {
        var currentKeys = Control.ModifierKeys;
        if(currentKeys == _lastModifierKeys) return;

        KeyStateDisplay ck = null, sk = null;

        lock (_lockObj)
        {
            if ((currentKeys & Keys.Control) == Keys.Control)
            {
                ck = new KeyStateDisplay(
                    "CONTROL",
                    new Point(Cursor.Position.X + 20, Cursor.Position.Y),
                    Color.Cyan
                );
                _activeKeyDisplays.Add(ck);
            }

            if ((currentKeys & Keys.Shift) == Keys.Shift)
            {
                sk = new KeyStateDisplay(
                    "SHIFT",
                    new Point(Cursor.Position.X + 20, Cursor.Position.Y + 25),
                    Color.Cyan
                );
                _activeKeyDisplays.Add(sk);
            }
        }
        ...
        Task.Delay(1000).ContinueWith(t =>
        {
            lock(_lockObj)
            {
                if (ck != null) _activeKeyDisplays.Remove(ck);
                if (sk != null) _activeKeyDisplays.Remove(sk);
            }
            this.Invalidate();
        }, TaskScheduler.FromCurrentSynchronizationContext());

        _lastModifierKeys = currentKeys;
        this.Invalidate();
    }*/
